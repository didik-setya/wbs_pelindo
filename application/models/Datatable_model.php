...
class Datatable_model extends CI_Model {
    ...
    private function get_report($relation = null){
        ...
        if($relation){
            $this->db->where('relation_all', $relation);
        }
    ...
    public function get_data_report($relation = null){
        $this->get_report($relation);
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    public function count_filtered_report($relation = null){
        $this->get_report($relation);
        $query = $this->db->get();
        return $query->num_rows();
    }

    public function count_all_report($relation = null){
        $this->db->from('laporan');
        if($relation){
            $this->db->where('relation_all', $relation);
        }
